...
if (!process.env.OPENAI_API_KEY) {
  console.error('[verbo-backend] OPENAI_API_KEY is not set. AI calls will fail.');
}
...
const collectURLCitations = (response: Response): GroundingSource[] => {
...
export const transcribeAudio = async (audioBase64: string, mimeType: string): Promise<string> => {
  ...
  const file = new File([binary], `recording.${ext}`, { type: mimeType });
...
export const SYNTHESIS_FALLBACK: ResearchSynthesis = {
  agree: '',
  disagree: '',
  underexplored: '',
  agreementPct: 0,
